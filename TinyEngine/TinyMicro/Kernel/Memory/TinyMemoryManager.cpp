...
native_pointer TinyMemoryManager::GetAddress( const tiny_storage& storage ) const {
	return GetAddress( storage.Block );
}

native_pointer TinyMemoryManager::GetAddress( tiny_uint block ) const {
	auto* blocks  = tiny_cast( _block_storage, TinyMemoryBlock* );
	auto* address = tiny_cast( nullptr, native_pointer );

	if ( block < MAX_BLOCKS ) {
		auto* offset = tiny_cast( blocks + MAX_BLOCKS, tiny_pointer );

		address = tiny_cast( offset + blocks[ block ].Offset, native_pointer );
	}
